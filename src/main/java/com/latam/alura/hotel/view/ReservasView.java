...
public class ReservasView extends JFrame {
	...
	private double valorReservaDouble;
	...
	public ReservasView() {
		...
		txtFechaSalida.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				// calcular el valor de la reservacion							
				if ("date".equals(evt.getPropertyName())) {					
					Date fechaAnterior = txtFechaEntrada.getDate();
					Date fechaPosterior = txtFechaSalida.getDate();					
					long diDernciaDias = fechaPosterior.getTime() - fechaAnterior.getTime();					
					TimeUnit time = TimeUnit.DAYS;
					long dif = time.convert(diDernciaDias, TimeUnit.MILLISECONDS);						
					
					if (fechaAnterior != null && fechaPosterior != null) {										
						if (dif >= 0) {							
							valorReservaDouble = (dif + 1) * 15.0;
							String valorReserva = String.valueOf(valorReservaDouble);							
		...
		btnsiguiente.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtFechaEntrada.getDate() != null && ReservasView.txtFechaSalida.getDate() != null) {
					...
					if(fechaEntrada.getTime() < fechaActual.getTime()) {					
						JOptionPane.showMessageDialog(null, "La fecha de Entrda no puede ser menor que la fecha actual!");
						txtFechaEntrada.requestFocusInWindow();						
						return;
					}
					if(fechaSalida.getTime() < fechaActual.getTime()) {
						JOptionPane.showMessageDialog(null, "La fecha Final no puede ser menor que la fecha actual!");
						txtFechaSalida.requestFocusInWindow();						
						return;
					}
					if(fechaSalida.getTime() < fechaEntrada.getTime()) {
						JOptionPane.showMessageDialog(null, "La fecha Final no puede ser menor que la fecha Inicial!");
						txtFechaSalida.requestFocusInWindow();						
						return;
					}					
					...
					registro.establecerNumeroReserva(numeroReservacion);				
	...
	private void agregarReservacion() {
		Date fechaInicial = txtFechaEntrada.getDate();		
		Date fechaFinal = txtFechaSalida.getDate();		
		
		String formaPago = txtFormaPago.getSelectedItem().toString();
		
		Reservacion miReservacion = new Reservacion(fechaInicial, fechaFinal, valorReservaDouble, formaPago);
		ReservacionController reservacion = new ReservacionController();
		numeroReservacion = reservacion.guardarReservacion(miReservacion);
